  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 pb-16">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="w-full h-full bg-gradient-to-br from-[#355E47] via-[#2D5F3F] to-[#3A5F4A]"></div>
        {mounted && (
          <div className="absolute inset-0 z-10">
            {[...Array(20)].map((_, i) => (
              <motion.div
                ...
                animate={{ y: [null, Math.random() * -100 - 50], opacity: [0.2, 0.5, 0.2] }}
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center w-full">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          {/* Main Heading — tightened from text-6xl to text-5xl max */}
          <motion.h1
            className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 md:mb-5 leading-tight px-4"
            key={currentText}
          ...
          {/* Subtext — matches other hero sections */}
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.3, duration: 0.8 }}
            className="text-sm sm:text-base md:text-lg text-white/90 mb-8 md:mb-10 max-w-3xl mx-auto px-4"
          >
          ...
          >
            <form onSubmit={handleSearch}>
              <div className="relative group">
                <Search className="absolute left-4 sm:left-6 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4 sm:w-5 sm:h-5 z-10" />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search programs, courses, or locations..."
                  className="w-full pl-10 sm:pl-14 pr-24 sm:pr-28 py-3 sm:py-3.5 rounded-full bg-white/95 backdrop-blur-sm text-gray-800 text-sm sm:text-base focus:outline-none focus:ring-4 focus:ring-[#355E47]/50 transition-all shadow-2xl"
                  autoComplete="off"
                />
                {searchQuery && (
                  <button
                    type="button"
                    onClick={clearSearch}
                    className="absolute right-24 sm:right-28 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1"
                  >
                    <X className="w-4 h-4" />
                  </button>
                )}
                <button
                  type="submit"
                  className="absolute right-2 top-1/2 -translate-y-1/2 bg-gradient-to-r from-[#355E47] to-[#2D5F3F] text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-full font-semibold text-xs sm:text-sm hover:from-[#2D5F3F] hover:to-[#3A5F4A] transition-all shadow-lg"
                >
                  Search
                </button>
              </div>
            ...
            {showResults && searchResults.length > 0 && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="absolute w-full mt-2 bg-white rounded-2xl shadow-2xl overflow-hidden z-50 max-h-80 overflow-y-auto"
              >
                {searchResults.map((result, index) => (
                  <div
                    key={index}
                    onClick={() => handleResultClick(result.url)}
                    className="flex items-center justify-between px-5 py-3 hover:bg-[#355E47]/5 cursor-pointer transition-colors border-b border-gray-100 last:border-b-0"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-gradient-to-br from-[#355E47]/20 to-[#2D5F3F]/20 rounded-lg flex items-center justify-center">
                        {result.type === 'Program' && <GraduationCap className="w-4 h-4 text-[#355E47]" />}
                        {result.type === 'Location' && <Globe className="w-4 h-4 text-[#2D5F3F]" />}
                        {result.type === 'Page' && <Search className="w-4 h-4 text-[#3A5F4A]" />}
                      </div>
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getBadgeColor(result.type)}`}>
                      {result.type}
                    </span>
                  </div>
                ))}
            ...
            {showResults && searchResults.length === 0 && searchQuery.trim() !== '' && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="absolute w-full mt-2 bg-white rounded-2xl shadow-2xl p-5 z-50 text-center"
              >
                <p className="text-gray-500 text-sm">No results found for "{searchQuery}"</p>
                <p className="text-xs text-gray-400 mt-1">Try searching for programs, locations, or pages</p>
              </motion.div>
            )}
          </motion.div>
          ...
          <motion.div
            ...
            className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-10 md:mb-12 px-4"
          >
            <button
              onClick={() => router.push('/apply')}
              className="group relative w-full sm:w-auto px-8 py-3 bg-gradient-to-r from-[#355E47] to-[#2D5F3F] text-white font-bold text-sm md:text-base rounded-full overflow-hidden shadow-2xl hover:shadow-[#355E47]/50 transition-all hover:scale-105"
            >
          ...
          >
            {quickStats.map((stat, index) => (
              <motion.div
                ...
                className="bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-5 border border-white/20 hover:bg-white/15 transition-all"
              >
                <stat.icon className="w-6 h-6 sm:w-7 sm:h-7 text-white/90 mx-auto mb-2" />
                {/* Stat value — matches StatsCounter sizing */}
                <div className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-1">{stat.value}</div>
                {/* Stat label — matches site standard xs/sm */}
